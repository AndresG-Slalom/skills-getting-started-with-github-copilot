          ...
          ? `<ul>${details.participants
              .map(
                (email) => `
                  <li>
                    <span>${email}</span>
                    <button class="remove-participant" type="button" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(email)}" aria-label="Remove ${email} from ${name}">x</button>
                  </li>`
              )
              .join("")}</ul>`
